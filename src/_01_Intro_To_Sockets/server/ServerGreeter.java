...
public class ServerGreeter extends Thread {
	...
	public void run() {
		...
		Socket acception = seSock.accept();
				...
				DataInputStream is = new DataInputStream(acception.getInputStream());
				//12. Print the message from the DataInputStream object using the readUTF() method
				System.out.println("Client sent a message: "+is.readUTF());
				...
				DataOutputStream os = new DataOutputStream(acception.getOutputStream());
				//14. Use the DataOutputStream object to send a message to the client using the writeUTF(String message) method.
				os.writeUTF(JOptionPane.showInputDialog("response tro client"));
